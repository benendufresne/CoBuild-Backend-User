...
import { ENVIRONMENT, GENERATOR, MIME_TYPE, REGEX, SERVER } from "@config/index";
import { logger } from "@lib/logger";
import * as QRCode from "qrcode";
const TAG = "cobuild_uploads";
...
const qrCodeGenerator = async (value: string) => {
  const filePath = process.cwd() + "/src/uploads/" + value + ".png";
  const qrCodePath = await QRCode.toFile(filePath, value, { type: 'png' });
  return qrCodePath;
}
