...
class App {
  ...
  private async startapp() {
    fetchSecrets().then(async () => {
      this.template();
      this.Port = process.env.USER_PORT;
  ...
  private callback = () => {
    logger.info(
      ` Hapi server swagger is running this URL :- ${SERVER.APP_URL}:${SERVER.USER_PORT}/${SERVER.USER_MICROSERVICE_URL}/documentation`
    );
